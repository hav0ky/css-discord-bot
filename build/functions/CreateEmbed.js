...
exports.calculateKDRatio = calculateKDRatio;
...
const CreateEmbed = (server) => {
    const embed = new discord_js_1.EmbedBuilder()
        ...
        .addFields({ name: 'Status', value: ':green_circle: Online', inline: true }, { name: 'Address:Port', value: '`' + server.address + '`', inline: true }, { name: 'Country', value: ':flag_in: IN', inline: true }, { name: 'Game', value: server.game, inline: true }, { name: 'Current Map', value: `${server.map}`, inline: true }, { name: 'Players', value: `${server.players.length}/${server.maxPlayers} (${server.playersPercentage}%)`, inline: true })
        // .addFields({ name: 'Inline field title', value: 'Some value here', inline: true })
        // .setImage('https://i.imgur.com/AfFp7pu.png')
        // .setTimestamp()
        .setFooter({ text: 'Powered by proaim.cc | Game Server Monitor' });
    if (server.players.length > 0) {
        server.players.map((player) => table.addRow([truncateString(player.playerName), `${calculateKDRatio(Number(player.k), Number(player.d))}`, `${player.k}`, `${player.d}`]));
